;--------------------------------------------------------
; File Created by SDCC : free open source ISO C Compiler
; Version 4.5.0 #15242 (Mac OS X ppc)
;--------------------------------------------------------
	.module Key
	
	.optsdcc -mmcs51 --model-small
;--------------------------------------------------------
; Public variables in this module
;--------------------------------------------------------
	.globl _KEY4
	.globl _KEY3
	.globl _KEY2
	.globl _KEY1
	.globl _CCF0
	.globl _CCF1
	.globl _CCF2
	.globl _CR
	.globl _CF
	.globl _RI
	.globl _TI
	.globl _RB8
	.globl _TB8
	.globl _REN
	.globl _SM2
	.globl _SM1
	.globl _SM0
	.globl _IT0
	.globl _IE0
	.globl _IT1
	.globl _IE1
	.globl _TR0
	.globl _TF0
	.globl _TR1
	.globl _TF1
	.globl _PX0
	.globl _PT0
	.globl _PX1
	.globl _PT1
	.globl _PS
	.globl _PADC
	.globl _PLVD
	.globl _PPCA
	.globl _EX0
	.globl _ET0
	.globl _EX1
	.globl _ET1
	.globl _ES
	.globl _EADC
	.globl _ELVD
	.globl _EA
	.globl _P77
	.globl _P76
	.globl _P75
	.globl _P74
	.globl _P73
	.globl _P72
	.globl _P71
	.globl _P70
	.globl _P67
	.globl _P66
	.globl _P65
	.globl _P64
	.globl _P63
	.globl _P62
	.globl _P61
	.globl _P60
	.globl _P57
	.globl _P56
	.globl _P55
	.globl _P54
	.globl _P53
	.globl _P52
	.globl _P51
	.globl _P50
	.globl _P47
	.globl _P46
	.globl _P45
	.globl _P44
	.globl _P43
	.globl _P42
	.globl _P41
	.globl _P40
	.globl _P37
	.globl _P36
	.globl _P35
	.globl _P34
	.globl _P33
	.globl _P32
	.globl _P31
	.globl _P30
	.globl _P27
	.globl _P26
	.globl _P25
	.globl _P24
	.globl _P23
	.globl _P22
	.globl _P21
	.globl _P20
	.globl _P17
	.globl _P16
	.globl _P15
	.globl _P14
	.globl _P13
	.globl _P12
	.globl _P11
	.globl _P10
	.globl _P07
	.globl _P06
	.globl _P05
	.globl _P04
	.globl _P03
	.globl _P02
	.globl _P01
	.globl _P00
	.globl _P
	.globl _F1
	.globl _OV
	.globl _RS0
	.globl _RS1
	.globl _F0
	.globl _AC
	.globl _CY
	.globl _PWMFDCR
	.globl _PWMIF
	.globl _PWMCR
	.globl _PWMCFG
	.globl _CMPCR2
	.globl _CMPCR1
	.globl _CCAP2H
	.globl _CCAP1H
	.globl _CCAP0H
	.globl _PCA_PWM2
	.globl _PCA_PWM1
	.globl _PCA_PWM0
	.globl _CCAP2L
	.globl _CCAP1L
	.globl _CCAP0L
	.globl _CCAPM2
	.globl _CCAPM1
	.globl _CCAPM0
	.globl _CH
	.globl _CL
	.globl _CMOD
	.globl _CCON
	.globl _IAP_CONTR
	.globl _IAP_TRIG
	.globl _IAP_CMD
	.globl _IAP_ADDRL
	.globl _IAP_ADDRH
	.globl _IAP_DATA
	.globl _SPDAT
	.globl _SPCTL
	.globl _SPSTAT
	.globl _ADC_RESL
	.globl _ADC_RES
	.globl _ADC_CONTR
	.globl _SADEN
	.globl _SADDR
	.globl _S4BUF
	.globl _S4CON
	.globl _S3BUF
	.globl _S3CON
	.globl _S2BUF
	.globl _S2CON
	.globl _SBUF
	.globl _SCON
	.globl _WDT_CONTR
	.globl _WKTCH
	.globl _WKTCL
	.globl _T2L
	.globl _T2H
	.globl _T3L
	.globl _T3H
	.globl _T4L
	.globl _T4H
	.globl _T3T4M
	.globl _T4T3M
	.globl _TH1
	.globl _TH0
	.globl _TL1
	.globl _TL0
	.globl _TMOD
	.globl _TCON
	.globl _INT_CLKO
	.globl _IP2
	.globl _IE2
	.globl _IP
	.globl _IE
	.globl _P_SW2
	.globl _P1ASF
	.globl _BUS_SPEED
	.globl _CLK_DIV
	.globl _P_SW1
	.globl _AUXR1
	.globl _AUXR
	.globl _PCON
	.globl _P7M1
	.globl _P7M0
	.globl _P6M1
	.globl _P6M0
	.globl _P5M1
	.globl _P5M0
	.globl _P4M1
	.globl _P4M0
	.globl _P3M1
	.globl _P3M0
	.globl _P2M1
	.globl _P2M0
	.globl _P1M1
	.globl _P1M0
	.globl _P0M1
	.globl _P0M0
	.globl _P7
	.globl _P6
	.globl _P5
	.globl _P4
	.globl _P3
	.globl _P2
	.globl _P1
	.globl _P0
	.globl _DPH
	.globl _DPL
	.globl _SP
	.globl _PSW
	.globl _B
	.globl _ACC
	.globl _Combination_14
	.globl _Combination_13
	.globl _Combination_12
	.globl _k2_long_flag
	.globl _k1_long_flag
	.globl _k4_short_flag
	.globl _k3_short_flag
	.globl _k2_short_flag
	.globl _k1_short_flag
	.globl _k4_cnt
	.globl _k3_cnt
	.globl _k2_cnt
	.globl _k1_cnt
	.globl _k4_lock_flage
	.globl _k3_lock_flage
	.globl _k2_lock_flage
	.globl _k1_lock_flage
	.globl _KeyNum
	.globl _POP_KEY
	.globl _Key_Loop
;--------------------------------------------------------
; special function registers
;--------------------------------------------------------
	.area RSEG    (ABS,DATA)
	.org 0x0000
_ACC	=	0x00e0
_B	=	0x00f0
_PSW	=	0x00d0
_SP	=	0x0081
_DPL	=	0x0082
_DPH	=	0x0083
_P0	=	0x0080
_P1	=	0x0090
_P2	=	0x00a0
_P3	=	0x00b0
_P4	=	0x00c0
_P5	=	0x00c8
_P6	=	0x00e8
_P7	=	0x00f8
_P0M0	=	0x0094
_P0M1	=	0x0093
_P1M0	=	0x0092
_P1M1	=	0x0091
_P2M0	=	0x0096
_P2M1	=	0x0095
_P3M0	=	0x00b2
_P3M1	=	0x00b1
_P4M0	=	0x00b4
_P4M1	=	0x00b3
_P5M0	=	0x00ca
_P5M1	=	0x00c9
_P6M0	=	0x00cc
_P6M1	=	0x00cb
_P7M0	=	0x00e2
_P7M1	=	0x00e1
_PCON	=	0x0087
_AUXR	=	0x008e
_AUXR1	=	0x00a2
_P_SW1	=	0x00a2
_CLK_DIV	=	0x0097
_BUS_SPEED	=	0x00a1
_P1ASF	=	0x009d
_P_SW2	=	0x00ba
_IE	=	0x00a8
_IP	=	0x00b8
_IE2	=	0x00af
_IP2	=	0x00b5
_INT_CLKO	=	0x008f
_TCON	=	0x0088
_TMOD	=	0x0089
_TL0	=	0x008a
_TL1	=	0x008b
_TH0	=	0x008c
_TH1	=	0x008d
_T4T3M	=	0x00d1
_T3T4M	=	0x00d1
_T4H	=	0x00d2
_T4L	=	0x00d3
_T3H	=	0x00d4
_T3L	=	0x00d5
_T2H	=	0x00d6
_T2L	=	0x00d7
_WKTCL	=	0x00aa
_WKTCH	=	0x00ab
_WDT_CONTR	=	0x00c1
_SCON	=	0x0098
_SBUF	=	0x0099
_S2CON	=	0x009a
_S2BUF	=	0x009b
_S3CON	=	0x00ac
_S3BUF	=	0x00ad
_S4CON	=	0x0084
_S4BUF	=	0x0085
_SADDR	=	0x00a9
_SADEN	=	0x00b9
_ADC_CONTR	=	0x00bc
_ADC_RES	=	0x00bd
_ADC_RESL	=	0x00be
_SPSTAT	=	0x00cd
_SPCTL	=	0x00ce
_SPDAT	=	0x00cf
_IAP_DATA	=	0x00c2
_IAP_ADDRH	=	0x00c3
_IAP_ADDRL	=	0x00c4
_IAP_CMD	=	0x00c5
_IAP_TRIG	=	0x00c6
_IAP_CONTR	=	0x00c7
_CCON	=	0x00d8
_CMOD	=	0x00d9
_CL	=	0x00e9
_CH	=	0x00f9
_CCAPM0	=	0x00da
_CCAPM1	=	0x00db
_CCAPM2	=	0x00dc
_CCAP0L	=	0x00ea
_CCAP1L	=	0x00eb
_CCAP2L	=	0x00ec
_PCA_PWM0	=	0x00f2
_PCA_PWM1	=	0x00f3
_PCA_PWM2	=	0x00f4
_CCAP0H	=	0x00fa
_CCAP1H	=	0x00fb
_CCAP2H	=	0x00fc
_CMPCR1	=	0x00e6
_CMPCR2	=	0x00e7
_PWMCFG	=	0x00f1
_PWMCR	=	0x00f5
_PWMIF	=	0x00f6
_PWMFDCR	=	0x00f7
;--------------------------------------------------------
; special function bits
;--------------------------------------------------------
	.area RSEG    (ABS,DATA)
	.org 0x0000
_CY	=	0x00d7
_AC	=	0x00d6
_F0	=	0x00d5
_RS1	=	0x00d4
_RS0	=	0x00d3
_OV	=	0x00d2
_F1	=	0x00d1
_P	=	0x00d0
_P00	=	0x0080
_P01	=	0x0081
_P02	=	0x0082
_P03	=	0x0083
_P04	=	0x0084
_P05	=	0x0085
_P06	=	0x0086
_P07	=	0x0087
_P10	=	0x0090
_P11	=	0x0091
_P12	=	0x0092
_P13	=	0x0093
_P14	=	0x0094
_P15	=	0x0095
_P16	=	0x0096
_P17	=	0x0097
_P20	=	0x00a0
_P21	=	0x00a1
_P22	=	0x00a2
_P23	=	0x00a3
_P24	=	0x00a4
_P25	=	0x00a5
_P26	=	0x00a6
_P27	=	0x00a7
_P30	=	0x00b0
_P31	=	0x00b1
_P32	=	0x00b2
_P33	=	0x00b3
_P34	=	0x00b4
_P35	=	0x00b5
_P36	=	0x00b6
_P37	=	0x00b7
_P40	=	0x00c0
_P41	=	0x00c1
_P42	=	0x00c2
_P43	=	0x00c3
_P44	=	0x00c4
_P45	=	0x00c5
_P46	=	0x00c6
_P47	=	0x00c7
_P50	=	0x00c8
_P51	=	0x00c9
_P52	=	0x00ca
_P53	=	0x00cb
_P54	=	0x00cc
_P55	=	0x00cd
_P56	=	0x00ce
_P57	=	0x00cf
_P60	=	0x00e8
_P61	=	0x00e9
_P62	=	0x00ea
_P63	=	0x00eb
_P64	=	0x00ec
_P65	=	0x00ed
_P66	=	0x00ee
_P67	=	0x00ef
_P70	=	0x00f8
_P71	=	0x00f9
_P72	=	0x00fa
_P73	=	0x00fb
_P74	=	0x00fc
_P75	=	0x00fd
_P76	=	0x00fe
_P77	=	0x00ff
_EA	=	0x00af
_ELVD	=	0x00ae
_EADC	=	0x00ad
_ES	=	0x00ac
_ET1	=	0x00ab
_EX1	=	0x00aa
_ET0	=	0x00a9
_EX0	=	0x00a8
_PPCA	=	0x00bf
_PLVD	=	0x00be
_PADC	=	0x00bd
_PS	=	0x00bc
_PT1	=	0x00bb
_PX1	=	0x00ba
_PT0	=	0x00b9
_PX0	=	0x00b8
_TF1	=	0x008f
_TR1	=	0x008e
_TF0	=	0x008d
_TR0	=	0x008c
_IE1	=	0x008b
_IT1	=	0x008a
_IE0	=	0x0089
_IT0	=	0x0088
_SM0	=	0x009f
_SM1	=	0x009e
_SM2	=	0x009d
_REN	=	0x009c
_TB8	=	0x009b
_RB8	=	0x009a
_TI	=	0x0099
_RI	=	0x0098
_CF	=	0x00df
_CR	=	0x00de
_CCF2	=	0x00da
_CCF1	=	0x00d9
_CCF0	=	0x00d8
_KEY1	=	0x0096
_KEY2	=	0x0097
_KEY3	=	0x0095
_KEY4	=	0x0094
;--------------------------------------------------------
; overlayable register banks
;--------------------------------------------------------
	.area REG_BANK_0	(REL,OVR,DATA)
	.ds 8
;--------------------------------------------------------
; internal ram data
;--------------------------------------------------------
	.area DSEG    (DATA)
_KeyNum::
	.ds 1
_k1_lock_flage::
	.ds 1
_k2_lock_flage::
	.ds 1
_k3_lock_flage::
	.ds 1
_k4_lock_flage::
	.ds 1
_k1_cnt::
	.ds 2
_k2_cnt::
	.ds 2
_k3_cnt::
	.ds 2
_k4_cnt::
	.ds 2
_k1_short_flag::
	.ds 1
_k2_short_flag::
	.ds 1
_k3_short_flag::
	.ds 1
_k4_short_flag::
	.ds 1
_k1_long_flag::
	.ds 1
_k2_long_flag::
	.ds 1
_Combination_12::
	.ds 1
_Combination_13::
	.ds 1
_Combination_14::
	.ds 1
;--------------------------------------------------------
; overlayable items in internal ram
;--------------------------------------------------------
	.area	OSEG    (OVR,DATA)
;--------------------------------------------------------
; indirectly addressable internal ram data
;--------------------------------------------------------
	.area ISEG    (DATA)
;--------------------------------------------------------
; absolute internal ram data
;--------------------------------------------------------
	.area IABS    (ABS,DATA)
	.area IABS    (ABS,DATA)
;--------------------------------------------------------
; bit data
;--------------------------------------------------------
	.area BSEG    (BIT)
;--------------------------------------------------------
; paged external ram data
;--------------------------------------------------------
	.area PSEG    (PAG,XDATA)
;--------------------------------------------------------
; uninitialized external ram data
;--------------------------------------------------------
	.area XSEG    (XDATA)
;--------------------------------------------------------
; absolute external ram data
;--------------------------------------------------------
	.area XABS    (ABS,XDATA)
;--------------------------------------------------------
; initialized external ram data
;--------------------------------------------------------
	.area XISEG   (XDATA)
	.area HOME    (CODE)
	.area GSINIT0 (CODE)
	.area GSINIT1 (CODE)
	.area GSINIT2 (CODE)
	.area GSINIT3 (CODE)
	.area GSINIT4 (CODE)
	.area GSINIT5 (CODE)
	.area GSINIT  (CODE)
	.area GSFINAL (CODE)
	.area CSEG    (CODE)
;--------------------------------------------------------
; global & static initialisations
;--------------------------------------------------------
	.area HOME    (CODE)
	.area GSINIT  (CODE)
	.area GSFINAL (CODE)
	.area GSINIT  (CODE)
;	code/key/Key.c:6: unsigned char KeyNum = 0;
	mov	_KeyNum,#0x00
;	code/key/Key.c:13: unsigned char k1_lock_flage = 0;
	mov	_k1_lock_flage,#0x00
;	code/key/Key.c:14: unsigned char k2_lock_flage = 0;
	mov	_k2_lock_flage,#0x00
;	code/key/Key.c:15: unsigned char k3_lock_flage = 0;
	mov	_k3_lock_flage,#0x00
;	code/key/Key.c:16: unsigned char k4_lock_flage = 0;
	mov	_k4_lock_flage,#0x00
;	code/key/Key.c:18: unsigned int k1_cnt = 0; // 单击按键基数
	clr	a
	mov	_k1_cnt,a
	mov	(_k1_cnt + 1),a
;	code/key/Key.c:19: unsigned int k2_cnt = 0; // 单击按键基数
	mov	_k2_cnt,a
	mov	(_k2_cnt + 1),a
;	code/key/Key.c:20: unsigned int k3_cnt = 0; // 单击按键基数
	mov	_k3_cnt,a
	mov	(_k3_cnt + 1),a
;	code/key/Key.c:21: unsigned int k4_cnt = 0; // 单击按键基数
	mov	_k4_cnt,a
	mov	(_k4_cnt + 1),a
;	code/key/Key.c:23: unsigned char k1_short_flag = 0; // 短按标志
	mov	_k1_short_flag,a
;	code/key/Key.c:24: unsigned char k2_short_flag = 0; // 短按标志
	mov	_k2_short_flag,a
;	code/key/Key.c:25: unsigned char k3_short_flag = 0; // 短按标志
	mov	_k3_short_flag,a
;	code/key/Key.c:26: unsigned char k4_short_flag = 0; // 短按标志
	mov	_k4_short_flag,a
;	code/key/Key.c:28: unsigned char k1_long_flag = 0; // 长按标志
	mov	_k1_long_flag,a
;	code/key/Key.c:29: unsigned char k2_long_flag = 0; // 长按标志
	mov	_k2_long_flag,a
;	code/key/Key.c:31: unsigned char Combination_12 = 0; // 组合按键按下标识
	mov	_Combination_12,a
;	code/key/Key.c:32: unsigned char Combination_13 = 0; // 组合按键按下标识
	mov	_Combination_13,a
;	code/key/Key.c:33: unsigned char Combination_14 = 0; // 组合按键按下标识
	mov	_Combination_14,a
;--------------------------------------------------------
; Home
;--------------------------------------------------------
	.area HOME    (CODE)
	.area HOME    (CODE)
;--------------------------------------------------------
; code
;--------------------------------------------------------
	.area CSEG    (CODE)
;------------------------------------------------------------
;Allocation info for local variables in function 'POP_KEY'
;------------------------------------------------------------
;temp          Allocated to registers 
;------------------------------------------------------------
;	code/key/Key.c:40: unsigned char POP_KEY(void)
;	-----------------------------------------
;	 function POP_KEY
;	-----------------------------------------
_POP_KEY:
	ar7 = 0x07
	ar6 = 0x06
	ar5 = 0x05
	ar4 = 0x04
	ar3 = 0x03
	ar2 = 0x02
	ar1 = 0x01
	ar0 = 0x00
;	code/key/Key.c:43: temp = KeyNum;
	mov	dpl,_KeyNum
;	code/key/Key.c:44: KeyNum = 0;
	mov	_KeyNum,#0x00
;	code/key/Key.c:45: return temp;
;	code/key/Key.c:46: }
	ret
;------------------------------------------------------------
;Allocation info for local variables in function 'Key_Loop'
;------------------------------------------------------------
;	code/key/Key.c:51: void Key_Loop(void)
;	-----------------------------------------
;	 function Key_Loop
;	-----------------------------------------
_Key_Loop:
;	code/key/Key.c:53: if (KEY1) // v+按键松开
	jnb	_KEY1,00133$
;	code/key/Key.c:56: k1_cnt = 0;
	clr	a
	mov	_k1_cnt,a
	mov	(_k1_cnt + 1),a
;	code/key/Key.c:57: if (k1_short_flag)
	mov	a,_k1_short_flag
	jz	00102$
;	code/key/Key.c:59: k1_short_flag = 0;
	mov	_k1_short_flag,#0x00
;	code/key/Key.c:60: KeyNum = 1;
	mov	_KeyNum,#0x01
00102$:
;	code/key/Key.c:62: if (k1_long_flag) // 在key1 长按的基础上 按住了key3
	mov	a,_k1_long_flag
	jz	00113$
;	code/key/Key.c:64: if (Combination_12)
	mov	a,_Combination_12
	jz	00110$
;	code/key/Key.c:66: Combination_12 = 0;
	mov	_Combination_12,#0x00
	sjmp	00111$
00110$:
;	code/key/Key.c:68: else if (Combination_13)
	mov	a,_Combination_13
	jz	00107$
;	code/key/Key.c:70: Combination_13 = 0;
	mov	_Combination_13,#0x00
	sjmp	00111$
00107$:
;	code/key/Key.c:72: else if (Combination_14)
	mov	a,_Combination_14
	jz	00104$
;	code/key/Key.c:74: Combination_14 = 0;
	mov	_Combination_14,#0x00
	sjmp	00111$
00104$:
;	code/key/Key.c:78: KeyNum = 11;
	mov	_KeyNum,#0x0b
00111$:
;	code/key/Key.c:80: k1_long_flag = 0;
	mov	_k1_long_flag,#0x00
00113$:
;	code/key/Key.c:83: k1_lock_flage = 0; // 清除自锁标志
	mov	_k1_lock_flage,#0x00
	ljmp	00134$
00133$:
;	code/key/Key.c:85: else if (!k1_lock_flage && !Combination_12 && !Combination_13 && !Combination_14) // 按键持续按下
	mov	a,_k1_lock_flage
	jz	00461$
	ljmp	00134$
00461$:
	mov	a,_Combination_12
	jnz	00134$
	mov	a,_Combination_13
	jnz	00134$
	mov	a,_Combination_14
	jnz	00134$
;	code/key/Key.c:87: if (++k1_cnt > KEY_DELAY_TIME)
	inc	_k1_cnt
	clr	a
	cjne	a,_k1_cnt,00465$
	inc	(_k1_cnt + 1)
00465$:
	clr	c
	mov	a,#0x14
	subb	a,_k1_cnt
	clr	a
	subb	a,(_k1_cnt + 1)
	jnc	00115$
;	code/key/Key.c:89: k1_short_flag = 1;
	mov	_k1_short_flag,#0x01
00115$:
;	code/key/Key.c:91: if (++k1_cnt > KEY_LONG_TIME)
	inc	_k1_cnt
	clr	a
	cjne	a,_k1_cnt,00467$
	inc	(_k1_cnt + 1)
00467$:
	clr	c
	mov	a,#0x58
	subb	a,_k1_cnt
	mov	a,#0x02
	subb	a,(_k1_cnt + 1)
	jnc	00134$
;	code/key/Key.c:93: if (!KEY2)
	jb	_KEY2,00123$
;	code/key/Key.c:95: k1_lock_flage = 1; // 自锁防止再次进入
	mov	_k1_lock_flage,#0x01
;	code/key/Key.c:96: k1_long_flag = 1;
	mov	_k1_long_flag,#0x01
;	code/key/Key.c:97: k1_short_flag = 0; // 清空短按
	mov	_k1_short_flag,#0x00
;	code/key/Key.c:98: Combination_12 = 1;
	mov	_Combination_12,#0x01
;	code/key/Key.c:99: KeyNum = 12;
	mov	_KeyNum,#0x0c
	sjmp	00134$
00123$:
;	code/key/Key.c:101: else if (!KEY3)
	jb	_KEY3,00120$
;	code/key/Key.c:103: k1_lock_flage = 1; // 自锁防止再次进入
	mov	_k1_lock_flage,#0x01
;	code/key/Key.c:104: k1_long_flag = 1;
	mov	_k1_long_flag,#0x01
;	code/key/Key.c:105: k1_short_flag = 0; // 清空短按
	mov	_k1_short_flag,#0x00
;	code/key/Key.c:106: Combination_13 = 1;
	mov	_Combination_13,#0x01
;	code/key/Key.c:107: KeyNum = 13;
	mov	_KeyNum,#0x0d
	sjmp	00134$
00120$:
;	code/key/Key.c:109: else if (!KEY4)
	jb	_KEY4,00117$
;	code/key/Key.c:111: k1_lock_flage = 1; // 自锁防止再次进入
	mov	_k1_lock_flage,#0x01
;	code/key/Key.c:112: k1_long_flag = 1;
	mov	_k1_long_flag,#0x01
;	code/key/Key.c:113: k1_short_flag = 0; // 清空短按
	mov	_k1_short_flag,#0x00
;	code/key/Key.c:114: Combination_14 = 1;
	mov	_Combination_14,#0x01
;	code/key/Key.c:115: KeyNum = 14;
	mov	_KeyNum,#0x0e
	sjmp	00134$
00117$:
;	code/key/Key.c:119: k1_short_flag = 0;
	mov	_k1_short_flag,#0x00
;	code/key/Key.c:120: k1_long_flag = 1;
	mov	_k1_long_flag,#0x01
00134$:
;	code/key/Key.c:125: if (KEY2) // v+按键松开
	jnb	_KEY2,00149$
;	code/key/Key.c:127: if (k2_short_flag)
	mov	a,_k2_short_flag
	jz	00136$
;	code/key/Key.c:129: k2_short_flag = 0;
	mov	_k2_short_flag,#0x00
;	code/key/Key.c:130: KeyNum = 2;
	mov	_KeyNum,#0x02
00136$:
;	code/key/Key.c:132: k2_lock_flage = 0; // 清除自锁标志
;	code/key/Key.c:133: k2_cnt = 0;
	clr	a
	mov	_k2_lock_flage,a
	mov	_k2_cnt,a
	mov	(_k2_cnt + 1),a
	sjmp	00150$
00149$:
;	code/key/Key.c:135: else if (!k2_lock_flage && !k1_short_flag && !k1_long_flag && !Combination_12 && !Combination_13 && !Combination_14) // 按键持续按下
	mov	a,_k2_lock_flage
	jnz	00150$
	mov	a,_k1_short_flag
	jnz	00150$
	mov	a,_k1_long_flag
	jnz	00150$
	mov	a,_Combination_12
	jnz	00150$
	mov	a,_Combination_13
	jnz	00150$
	mov	a,_Combination_14
	jnz	00150$
;	code/key/Key.c:137: if (++k2_cnt > KEY_DELAY_TIME)
	inc	_k2_cnt
	clr	a
	cjne	a,_k2_cnt,00480$
	inc	(_k2_cnt + 1)
00480$:
	clr	c
	mov	a,#0x14
	subb	a,_k2_cnt
	clr	a
	subb	a,(_k2_cnt + 1)
	jnc	00138$
;	code/key/Key.c:139: k2_short_flag = 2;
	mov	_k2_short_flag,#0x02
00138$:
;	code/key/Key.c:141: if (++k2_cnt > KEY_LONG_TIME)
	inc	_k2_cnt
	clr	a
	cjne	a,_k2_cnt,00482$
	inc	(_k2_cnt + 1)
00482$:
	clr	c
	mov	a,#0x58
	subb	a,_k2_cnt
	mov	a,#0x02
	subb	a,(_k2_cnt + 1)
	jnc	00150$
;	code/key/Key.c:143: k2_short_flag = 0;
	mov	_k2_short_flag,#0x00
;	code/key/Key.c:144: k2_lock_flage = 2;
	mov	_k2_lock_flage,#0x02
;	code/key/Key.c:145: KeyNum = 22;
	mov	_KeyNum,#0x16
00150$:
;	code/key/Key.c:150: if (KEY3) // v+按键松开
	jnb	_KEY3,00165$
;	code/key/Key.c:152: if (k3_short_flag)
	mov	a,_k3_short_flag
	jz	00152$
;	code/key/Key.c:154: k3_short_flag = 0;
	mov	_k3_short_flag,#0x00
;	code/key/Key.c:155: KeyNum = 3;
	mov	_KeyNum,#0x03
00152$:
;	code/key/Key.c:157: k3_lock_flage = 0; // 清除自锁标志
;	code/key/Key.c:158: k3_cnt = 0;
	clr	a
	mov	_k3_lock_flage,a
	mov	_k3_cnt,a
	mov	(_k3_cnt + 1),a
	sjmp	00166$
00165$:
;	code/key/Key.c:160: else if (!k3_lock_flage && !k1_short_flag && !k1_long_flag && !Combination_12 && !Combination_13 && !Combination_14) // 按键持续按下
	mov	a,_k3_lock_flage
	jnz	00166$
	mov	a,_k1_short_flag
	jnz	00166$
	mov	a,_k1_long_flag
	jnz	00166$
	mov	a,_Combination_12
	jnz	00166$
	mov	a,_Combination_13
	jnz	00166$
	mov	a,_Combination_14
	jnz	00166$
;	code/key/Key.c:162: if (++k3_cnt > KEY_DELAY_TIME)
	inc	_k3_cnt
	clr	a
	cjne	a,_k3_cnt,00492$
	inc	(_k3_cnt + 1)
00492$:
	clr	c
	mov	a,#0x14
	subb	a,_k3_cnt
	clr	a
	subb	a,(_k3_cnt + 1)
	jnc	00154$
;	code/key/Key.c:164: k3_short_flag = 1;
	mov	_k3_short_flag,#0x01
00154$:
;	code/key/Key.c:166: if (++k3_cnt > KEY_LONG_TIME)
	inc	_k3_cnt
	clr	a
	cjne	a,_k3_cnt,00494$
	inc	(_k3_cnt + 1)
00494$:
	clr	c
	mov	a,#0x58
	subb	a,_k3_cnt
	mov	a,#0x02
	subb	a,(_k3_cnt + 1)
	jnc	00166$
;	code/key/Key.c:168: k3_short_flag = 0;
	mov	_k3_short_flag,#0x00
;	code/key/Key.c:169: k3_lock_flage = 1; // Self-lock to prevent re-entry
	mov	_k3_lock_flage,#0x01
;	code/key/Key.c:170: KeyNum = 33;
	mov	_KeyNum,#0x21
00166$:
;	code/key/Key.c:174: if (KEY4) // v+按键松开
	jnb	_KEY4,00181$
;	code/key/Key.c:176: if (k4_short_flag)
	mov	a,_k4_short_flag
	jz	00168$
;	code/key/Key.c:178: k4_short_flag = 0;
	mov	_k4_short_flag,#0x00
;	code/key/Key.c:179: KeyNum = 4;
	mov	_KeyNum,#0x04
00168$:
;	code/key/Key.c:181: k4_lock_flage = 0; // 清除自锁标志
;	code/key/Key.c:182: k4_cnt = 0;
	clr	a
	mov	_k4_lock_flage,a
	mov	_k4_cnt,a
	mov	(_k4_cnt + 1),a
	ret
00181$:
;	code/key/Key.c:184: else if (!k4_lock_flage && !k1_short_flag && !k1_long_flag && !Combination_12 && !Combination_13 && !Combination_14) // 按键持续按下
	mov	a,_k4_lock_flage
	jnz	00183$
	mov	a,_k1_short_flag
	jnz	00183$
	mov	a,_k1_long_flag
	jnz	00183$
	mov	a,_Combination_12
	jnz	00183$
	mov	a,_Combination_13
	jnz	00183$
	mov	a,_Combination_14
	jnz	00183$
;	code/key/Key.c:186: if (++k4_cnt > KEY_DELAY_TIME)
	inc	_k4_cnt
	clr	a
	cjne	a,_k4_cnt,00504$
	inc	(_k4_cnt + 1)
00504$:
	clr	c
	mov	a,#0x14
	subb	a,_k4_cnt
	clr	a
	subb	a,(_k4_cnt + 1)
	jnc	00170$
;	code/key/Key.c:188: k4_short_flag = 1;
	mov	_k4_short_flag,#0x01
00170$:
;	code/key/Key.c:190: if (++k4_cnt > KEY_LONG_TIME)
	inc	_k4_cnt
	clr	a
	cjne	a,_k4_cnt,00506$
	inc	(_k4_cnt + 1)
00506$:
	clr	c
	mov	a,#0x58
	subb	a,_k4_cnt
	mov	a,#0x02
	subb	a,(_k4_cnt + 1)
	jnc	00183$
;	code/key/Key.c:192: k4_short_flag = 0;
	mov	_k4_short_flag,#0x00
;	code/key/Key.c:193: k4_lock_flage = 1; // Self-lock to prevent re-entry
	mov	_k4_lock_flage,#0x01
;	code/key/Key.c:194: KeyNum = 44;
	mov	_KeyNum,#0x2c
00183$:
;	code/key/Key.c:197: }
	ret
	.area CSEG    (CODE)
	.area CONST   (CODE)
	.area XINIT   (CODE)
	.area CABS    (ABS,CODE)
